...
export async function POST(request: NextRequest) {
  try {
    ...
    const mailOptions = {
      from: process.env.SMTP_FROM || '"Site Web Sophie Zen" <noreply@sophie-zen.fr>',
      to: process.env.SMTP_TO || 'contact@sophie-zen.fr',
    ...
    const confirmationMail = {
      from: process.env.SMTP_FROM || '"Sophie Zen" <noreply@sophie-zen.fr>',
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      console.log('Confirmation email would be sent in production:', confirmationMail)
    }
